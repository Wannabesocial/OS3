...
int main(int argc, char **argv){
    ...
    int q_block_size = atoi(argv[P_BLOCK_SIZE]), max_rows = atoi(argv[P_MAX_ROWS]), p = atoi(argv[P_POTITION]);
    const char *file_name = TRACE_FILE[p];
    ...
    for(int i = 0; i < total_reads1; i++){
        ...
        for(int i = 0; i < total_reads2; i++){
            ...
            shared_memory->last_chunk = (i+1 == total_reads2) ? true : false;
            cf_read(shared_memory->buffer, block_size2, trace_file); // Just Call Read function with BLOCK_SIZE2 XD!
